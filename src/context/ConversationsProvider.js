...
export function ConversationsProvider({ username, children }) {
	...
	const formattedConversations = conversations.map((conversation, index) => {
		const recipients = conversation.recipients.map((recipient) => {
			...
			const fullName = (contact && contact.fullName) || recipient;
			const profileImageURL = (contact && contact.profileImageURL) || recipient;
			return { username: recipient, fullName, profileImageURL };
		});
